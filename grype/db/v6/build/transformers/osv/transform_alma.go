...
func (almaStrategy) Transform(vuln unmarshal.OSVVulnerability, state provider.State) ([]data.Entry, error) {
	...
	in := []any{
		db.VulnerabilityHandle{
			...
			ModifiedDate:  vuln.Modified.AsTime(),
			PublishedDate: vuln.Published.AsTime(),
...
func almaUnaffectedBlob(vuln unmarshal.OSVVulnerability, affected osvmodel.AffectedPackage) *db.PackageBlob {
	var ranges []db.Range
...
func extractRpmModularity(affected osvmodel.AffectedPackage) string {
